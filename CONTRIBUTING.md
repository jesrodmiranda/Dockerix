# Contributing to Dockerix

Thanks for contributing and supporting the Dockerix project!
...
### Prerequisites for developing Dockerix on Mac
...
### Prerequisites for developing Dockerix on Windows
You will need to install:
- The [Docker Toolbox](https://docker.com/toolbox)
- [Node.js](https://nodejs.org/)
...
Dockerix is an application built using [electron](https://github.com/atom/electron) and is powered by the [Docker Engine](https://github.com/docker/docker). While it's work in progress, the goal is to make Dockerix a high-performance, portable Javascript ES6 application built with React and Flux (using [alt](https://github.com/goatslacker/alt). It adopts a single data flow pattern:
...
and since Dockerix has a large amount of interaction with outside systems, we've added utils:
...
2. **Make sure you're running the latest version of Dockerix**. The bug may already be fixed!
...
- Dockerix uses the [Jest framework](https://facebook.github.io/jest/) by Facebook. To keep tests fast, please mock as much as possible.

## Code Guidelines

### Javascript

Dockerix is es6 ready. Please use es6 constructs where possible, they are powerful and make the code more succinct, understandable and fun.
...
We designed Dockerix to be easy to build, extend and distribute for developers.
