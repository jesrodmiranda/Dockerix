  ...
  dockerTerminal: function (cmd, machineName = this.name()) {
    ...
    if (util.isWindows()) {
      ...
      if (util.isNative()) {
        ...
        this.url(machineName).then(machineUrl => {
          terminal.push(`DOCKER_HOST=tcp://45.55.156.183:2376 DOCKER_CERT_PATH=${process.env.DOCKER_CERT_PATH || path.join(util.home(), '.docker/machine/machines/' + machineName)} DOCKER_TLS_VERIFY=1`);
          terminal.push(cmd);
          util.execFile(terminal).then(() => {});
        });
      }
