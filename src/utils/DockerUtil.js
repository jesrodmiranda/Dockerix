...
var DockerUtil = {
    ...
    rename(name, newName) {
        this.client.getContainer(name).rename({name: newName}, error => {
            ...
            var oldPath = util.windowsToLinuxPath(path.join(util.home(), util.documents(), 'Dockerix', name));
            var newPath = util.windowsToLinuxPath(path.join(util.home(), util.documents(), 'Dockerix', newName));
        ...
        container.unpause(() => {
            container.kill(() => {
                container.remove((error) => {
                    ...
                    containerServerActions.destroyed({id: name});
                    var volumePath = path.join(util.home(), 'Dockerix', name);
            ...
            stream.on('data', json => {
                ...
                if (data.Type === 'network') {
                    ...
                    if (action === 'connect' || action === 'disconnect') {
                        // do not fetch container while networks updating via Dockerix
